...
export default function Home() {
  ...
  return <main className="studio-shell">
    ...
    <section className="workspace"><div className="score-panel"><div className="panel-head"><div><span className="eyebrow">PARTITUR</span><h1>Neue Idee <span>• {KEYS[keyName].label} • 4/4</span></h1></div><div className="legend"><span><i className="velocity" /> Velocity</span><span><i className="quant" /> quantisiert</span></div></div><div className="score"><div className="clef treble">𝄞</div><div className="clef bass">𝄢</div>{[0,1,2,3,4].map(i => <div key={`t${i}`} className="staff-line" style={{top: 32 + i * 13}} />)}{[0,1,2,3,4].map(i => <div key={`b${i}`} className="staff-line" style={{top: 125 + i * 13}} />)}{Array.from({length: Math.max(0, timelineBeats / 4 - 1)}, (_, i) => (i + 1) * 4).map(beat => <div key={beat} className="barline" style={{left: `${10 + beat / timelineBeats * 90}%`}} />)}{Array.from({length: Math.abs(KEYS[keyName].fifths)}, (_, i) => <span key={`ks-${i}`} className="key-signature-mark" style={{left: `${7.1 + i * .65}%`, top: (KEYS[keyName].fifths > 0 ? [32,51.5,25.5,45,64.5,38.5,58] : [58,38.5,64.5,45,71,51.5,77])[i]}}>{KEYS[keyName].fifths > 0 ? '♯' : '♭'}</span>)}{Array.from({length: Math.abs(KEYS[keyName].fifths)}, (_, i) => <span key={`ksb-${i}`} className="key-signature-mark" style={{left: `${7.1 + i * .65}%`, top: (KEYS[keyName].fifths > 0 ? [138,157.5,131.5,151,170.5,144.5,164] : [164,144.5,170.5,151,177,157.5,183.5])[i]}}>{KEYS[keyName].fifths > 0 ? '♯' : '♭'}</span>)}{notes.flatMap(n => {
      const left = 10 + n.start / timelineBeats * 90, downbeatOffset = Math.abs(n.start % 4) < .001 ? 10 : 0, y = scoreY(n.pitch, keyName), accidental = spelledNote(n.pitch, keyName).accidental;
      const rhythm = n.duration <= .125 ? 'thirty-second' : n.duration <= .25 ? 'sixteenth' : n.duration <= .5 ? 'eighth' : n.duration >= 2 ? 'half' : 'quarter';
      return [
        ...ledgerLines(n.pitch, keyName).map(line => <span key={`ledger-${n.id}-${line}`} className="ledger-line" style={{left: `calc(${left}% + ${downbeatOffset}px)`, top: line}} />),
        ...(accidental ? [<i key={`acc-${n.id}`} className="accidental-mark" style={{left: `calc(${left}% + ${downbeatOffset - 18}px)`, top: y}}>{accidental}</i>] : []),
        <button key={n.id} aria-label={`${pitchName(n.pitch)}, Velocity ${n.velocity}`} className={`score-note ${rhythm} ${selected === n.id ? 'selected' : ''}`} onClick={() => setSelected(n.id)} style={{left: `calc(${left}% + ${downbeatOffset}px)`, top: y, opacity: .55 + n.velocity / 280}}><span /></button>
      ];
    })}<div className="playhead" style={{left: `${10 + position / timelineBeats * 90}%`}} /></div></div>
