...
function diatonicIndex(p: number) { return (Math.floor(p / 12) - 1) * 7 + DIATONIC_STEPS[p % 12]; }
function scoreY(p: number) {
  // Treble: bottom line E4 = 84px. Bass: bottom line G2 = 177px.
  return p >= 60 ? 84 - (diatonicIndex(p) - 30) * 6.5 : 177 - (diatonicIndex(p) - 18) * 6.5;
}
function ledgerLines(p: number) {
  const y = scoreY(p), lines: number[] = [], top = p >= 60 ? 32 : 125, bottom = p >= 60 ? 84 : 177;
...
export default function Home() {
  ...
  return <main className="studio-shell">
    ...
    <section className="workspace"><div className="score-panel"><div className="panel-head"><div><span className="eyebrow">PARTITUR</span><h1>Neue Idee <span>• 4/4</span></h1></div><div className="legend"><span><i className="velocity" /> Velocity</span><span><i className="quant" /> quantisiert</span></div></div><div className="score"><div className="clef treble">𝄞</div><div className="clef bass">𝄢</div>{[0,1,2,3,4].map(i => <div key={`t${i}`} className="staff-line" style={{top: 32 + i * 13}} />)}{[0,1,2,3,4].map(i => <div key={`b${i}`} className="staff-line" style={{top: 125 + i * 13}} />)}{[4,8,12].map(beat => <div key={beat} className="barline" style={{left: `${10 + beat / TOTAL_BEATS * 88}%`}} />)}{notes.flatMap(n => {
      const left = 11 + n.start / TOTAL_BEATS * 86, y = scoreY(n.pitch);
      return [
        ...ledgerLines(n.pitch).map(line => <span key={`ledger-${n.id}-${line}`} className="ledger-line" style={{left: `${left}%`, top: line}} />),
        <button key={n.id} aria-label={`${pitchName(n.pitch)}, Velocity ${n.velocity}`} className={`score-note ${selected === n.id ? 'selected' : ''}`} onClick={() => setSelected(n.id)} style={{left: `${left}%`, top: y, opacity: .55 + n.velocity / 280}}>{NOTE_NAMES[n.pitch % 12].includes('#') && <i className="accidental">♯</i>}<span /></button>
      ];
    })}<div className="playhead" style={{left: `${10 + position / TOTAL_BEATS * 88}%`}} /></div></div>
      <div className="editor-panel"><div className="editor-tools"><div><span className="eyebrow">PIANO ROLL</span><strong>{notes.length} Noten</strong></div>{selectedNote && <div className="note-inspector"><span>{pitchName(selectedNote.pitch)}</span><label>Velocity <input type="range" min="1" max="127" value={selectedNote.velocity} onChange={e => setNotes(old => old.map(n => n.id === selected ? {...n, velocity: Number(e.target.value)} : n))}/><b>{selectedNote.velocity}</b></label><button onClick={() => { setNotes(old => old.filter(n => n.id !== selected)); setSelected(null); }}>Löschen</button></div>}<span className="hint">Scrollen für weitere Oktaven</span></div><div className="roll-wrap"><div className="piano-keys">{rows.map(p => <div key={p} className={NOTE_NAMES[p % 12].includes('#') ? 'key black' : 'key'}><span>{NOTE_NAMES[p % 12] === 'C' ? pitchName(p) : ''}</span></div>)}</div><div className="roll-grid">{rows.map(p => <div key={p} className={`grid-row ${NOTE_NAMES[p % 12].includes('#') ? 'dark' : ''}`} />)}{notes.map(n => n.pitch >= 48 && n.pitch <= 96 && <button key={n.id} onClick={() => { setSelected(n.id); playTone(n.pitch, n.velocity, .6); }} className={`note-block ${selected === n.id ? 'selected' : ''}`} style={{left: `${n.start / TOTAL_BEATS * 100}%`, width: `${Math.max(n.duration / TOTAL_BEATS * 100, 1.5)}%`, top: `${(96 - n.pitch) / rows.length * 100}%`, height: `${100 / rows.length}%`, '--vel': n.velocity / 127} as React.CSSProperties}><span>{pitchName(n.pitch)}</span></button>)}</div></div></div>
